...
func LeaveMessageCommand(ctx CommandHandler.Context, _ []string) error {
	...
	if sql.CheckLanguage(ctx.Guild.ID) == "tr" {
		...
		if err == nil {
			...
			insert, err := db.Query("INSERT INTO leavemessage (message, guildid) VALUES ('" + strings.Join(multiplexer.GetArgs(ctx.Message.Content, multiplexer.GetPrefix()), " ") + "', '" + ctx.Guild.ID + "')")
			if err != nil {
