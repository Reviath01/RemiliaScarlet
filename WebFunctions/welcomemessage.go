...
func WelcomeMessageHandler(c *gin.Context, session *discordgo.Session, conf *oauth2.Config) {
	val, _ := c.Cookie("key")
	switch val {
	case "":
	...
	default:
		...
		if err != nil {
			...
			message := c.Request.FormValue("welcomemessage")
			db := sql.Connect()
			defer db.Close()
			insert, _ := db.Query(fmt.Sprintf("INSERT INTO welcomemessage (message, guildid) VALUES ('%s', '%s')", message, guild.ID))
			defer insert.Close()
