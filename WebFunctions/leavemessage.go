...
func LeaveMessageHandler(c *gin.Context, session *discordgo.Session, conf *oauth2.Config) {
	val, _ := c.Cookie("key")
	switch val {
	case "":
	...
	default:
		...
		if err != nil {
			...
			message := c.Request.FormValue("leavemessage")
			db := sql.Connect()
			defer db.Close()
			insert, _ := db.Query(fmt.Sprintf("INSERT INTO leavemessage (message, guildid) VALUES ('%s', '%s')", message, guild.ID))
			defer insert.Close()
