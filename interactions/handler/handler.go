...
func InteractionHandler(session *discordgo.Session, event *discordgo.Event) {
	if event.Type == "INTERACTION_CREATE" {
		...
		if interaction.Data.Name == "settings" {
			response := interaction_commands.SettingsCommand(session, interaction)
			err = interaction.Respond(session, response)
			if err != nil {
				return
			}
		}
		if interaction.Data.Name == "slap" {
			response := interaction_commands.SlapCommand(session, interaction)
			err = interaction.Respond(session, response)
			if err != nil {
				return
			}
		...
		if interaction.Data.Name == "unban" {
			response := interaction_commands.UnbanCommand(session, interaction)
			err = interaction.Respond(session, response)
			if err != nil {
				return
			}
